...
namespace Cheburashka
{
    ...
    public sealed class EnforceForeignKeyRule: SqlCodeAnalysisRule
    {
        ...
        public override IList<SqlRuleProblem> Analyze(SqlRuleExecutionContext ruleExecutionContext)
        {
            ...
            var IsAutoGeneratedHistoryTable = modelElement.ObjectType.Relationships.Where(n => n.Name == "TemporalSystemVersioningHistoryTable" && n.Properties.Count > 0 ).ToList().Count;
            //If its created behind the scenes by SSDT - just return w/o further checking


            //if ( modelElement.te)
            //if (IsAutoGeneratedHistoryTable > 0 )
            //{
            //    return problems;
            //}
            ...
            var allTables = DMVSettings.GetTables;// model.GetObjects(DacQueryScopes.UserDefined, ForeignKeyConstraint.TypeClass).ToList();
            ...
            {
                var allProps = t.GetType().GetProperties(BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Instance).OrderBy(pi => pi.Name).ToList();
                var allFields = t.GetType().GetFields(BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Instance).OrderBy(pi => pi.Name).ToList();
                var x = 1;
            }
