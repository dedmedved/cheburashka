...
namespace Cheburashka
{
    ...
    /// whenever there is a temp table used in a routine, but not created in that routine.
    /// </para>
    /// <para>
    ...
    [LocalizedExportCodeAnalysisRule(AvoidUsingNonLocalTempTableRule.RuleId,
        RuleConstants.ResourceBaseName,                                     // Name of the resource file to look up displayname and description in
        RuleConstants.AvoidUsingNonLocalTempTableRuleName,                  // ID used to look up the display name inside the resources file
        RuleConstants.AvoidUsingNonLocalTempTableProblemDescription,        // ID used to look up the description inside the resources file
        Category = RuleConstants.CategoryTempTables,                        // Rule category (e.g. "Design", "Naming")
        RuleScope = SqlRuleScope.Element)]                                  // This rule targets specific elements rather than the whole model
    public sealed class AvoidUsingNonLocalTempTableRule : SqlCodeAnalysisRule
    {
        /// <summary>
        /// The Rule ID should resemble a fully-qualified class name. In the Visual Studio UI
        /// rules are grouped by "Namespace + Category", and each rule is shown using "Short ID: DisplayName".
        /// For this rule, it will be 
        /// shown as "DM0063: It's best to only use temp tables created and disposed of in the same block of code."
        /// </summary>
        public const string RuleId = RuleConstants.AvoidUsingNonLocalTempTableRuleId;

        public AvoidUsingNonLocalTempTableRule()
        {
