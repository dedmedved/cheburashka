...
namespace Cheburashka
{
    ...
    /// whenever there is a temp table in a routine which is never populated with data.
    /// </para>
    /// <para>
    ...
    [LocalizedExportCodeAnalysisRule(AvoidReadOnlyTempTableRule.RuleId,
        RuleConstants.ResourceBaseName,                                     // Name of the resource file to look up displayname and description in
        RuleConstants.AvoidReadOnlyTempTableRuleName,                       // ID used to look up the display name inside the resources file
        RuleConstants.AvoidReadOnlyTempTableProblemDescription,             // ID used to look up the description inside the resources file
        Category = RuleConstants.CategoryTempTables,                        // Rule category (e.g. "Design", "Naming")
        RuleScope = SqlRuleScope.Element)]                                  // This rule targets specific elements rather than the whole model
    public sealed class AvoidReadOnlyTempTableRule : SqlCodeAnalysisRule
    {
        /// <summary>
        /// The Rule ID should resemble a fully-qualified class name. In the Visual Studio UI
        /// rules are grouped by "Namespace + Category", and each rule is shown using "Short ID: DisplayName".
        /// For this rule, it will be 
        /// shown as "DM0065: Temp Tables should be populated with data."
        /// </summary>
        public const string RuleId = RuleConstants.AvoidReadOnlyTempTableRuleId;

        public AvoidReadOnlyTempTableRule()
        {
