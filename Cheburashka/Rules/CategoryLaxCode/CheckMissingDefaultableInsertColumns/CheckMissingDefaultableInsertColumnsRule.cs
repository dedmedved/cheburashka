...
namespace Cheburashka
{
    ...
    /// whenever there is an insert that omits nullable or defaultable columns
    /// </para>
    /// <para>
    ...
    [LocalizedExportCodeAnalysisRule(CheckMissingDefaultableInsertColumnsRule.RuleId,
        RuleConstants.ResourceBaseName,                                         // Name of the resource file to look up display name and description in
        RuleConstants.CheckMissingDefaultableInsertColumnsRuleName,             // ID used to look up the display name inside the resources file
        RuleConstants.CheckMissingDefaultableInsertColumnsProblemDescription,   // ID used to look up the description inside the resources file
        Category = RuleConstants.CategoryNonStrictCodingStyle,                  // Rule category (e.g. "Design", "Naming")
        RuleScope = SqlRuleScope.Element)]                                      // This rule targets specific elements rather than the whole model
    public sealed class CheckMissingDefaultableInsertColumnsRule : SqlCodeAnalysisRule
    {
        /// <summary>
        /// <para>
        ...
        /// shown as "DM0066: Columns should be populated without relying on NULL or default values in Insert operations."
        /// </para>
        /// </summary>
        public const string RuleId = RuleConstants.CheckMissingDefaultableInsertColumnsRuleId;

        public CheckMissingDefaultableInsertColumnsRule()
        {
        ...
        private static void FindMissingMandatoryColumns(VariableTableReference table, IEnumerable<TableDetails> allTables, IReadOnlyCollection<ColumnReferenceExpression> insertcolumns, ICollection<(TSqlFragment,string)> problemInserts, TSqlFragment fragment)
        {
            ...
            var defaultableColumns = columns.Where(c =>  ( c.Constraints.Any( n => n is NullableConstraintDefinition { Nullable: true })   ||
                                                           ! c.Constraints.Any( n => n is NullableConstraintDefinition)                    || //assumes no nullability = NULL ??? 
                                                           c.DefaultConstraint is not null)
                                                         && c.ComputedColumnExpression    is null
                                                         && c.IdentityOptions             is null 
                                                         && ! c.GeneratedAlways.HasValue
            ...
            var problems = defaultableColumns.Where(c => !insertcolumns.Any(ic =>
        ...
        private static void FindMissingMandatoryColumns(NamedTableReference table, IEnumerable<TSqlObject> allTables, IReadOnlyCollection<ColumnReferenceExpression> insertcolumns, ICollection<(TSqlFragment,string)> problemInserts, TSqlFragment fragment)
        {
            ...
            var defaultableColumns = columns.Where(c => ( (bool)c.GetProperty(Column.Nullable)                                             ||
                                                          c.GetReferencingRelationshipInstances(DefaultConstraint.TargetColumn).Any()
                                                        )
                                                        &&  c.GetProperty(Column.Expression) is null   
