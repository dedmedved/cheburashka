...
namespace Cheburashka
{
    ...
    [LocalizedExportCodeAnalysisRule( CheckUniqueFilteredIndexImpliedBySubQueryRule.RuleId,
        RuleConstants.ResourceBaseName,                                             // Name of the resource file to look up display name and description in
        RuleConstants.CheckUniqueFilteredIndexImpliedBySubQueryRuleName,            // ID used to look up the display name inside the resources file
        RuleConstants.CheckUniqueFilteredIndexImpliedBySubQueryProblemDescription,  // ID used to look up the description inside the resources file
        Category = RuleConstants.CategoryMissingDerivedConstraints,                 // Rule category (e.g. "Design", "Naming")
        RuleScope = SqlRuleScope.Element)]                                          // This rule targets specific elements rather than the whole model
    public sealed class CheckUniqueFilteredIndexImpliedBySubQueryRule : SqlCodeAnalysisRule
    {
        /// <summary>
        /// The Rule ID should resemble a fully-qualified class name. In the Visual Studio UI
        /// rules are grouped by "Namespace + Category", and each rule is shown using "Short ID: DisplayName".
        /// For this rule, it will be 
        /// shown as "DM0073: Single table sub-queries with and-ed equality filters with other conditions imply a unique filtered index on the source data."
        /// </summary>
        public const string RuleId = RuleConstants.CheckUniqueFilteredIndexImpliedBySubQueryRuleId;

        public CheckUniqueFilteredIndexImpliedBySubQueryRule()
        {
